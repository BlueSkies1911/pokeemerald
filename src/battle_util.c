...
void HandleAction_UseMove(void)
{
    ...
    moveTarget = gMovesInfo[gCurrentMove].target;
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_MOVES_BLOCK: // 2
        {
            u16 moveTarget = gMovesInfo[move].target;
...
static u8 StatRaiseBerry(u32 battler, u32 itemId, u32 statId, bool32 end2)
{
    ...
    {
        BufferStatChange(statId, STRINGID_STATROSE);
        gEffectBattler = battler;
        SET_STATCHANGER(statId, 1, FALSE);
...
static u8 DamagedStatBoostBerryEffect(u32 battler, u8 statId, u8 category)
{
    if (IsBattlerAlive(battler)
     ...
     && (gBattleScripting.overrideBerryRequirements
         || (!DoesSubstituteBlockMove(gBattlerAttacker, battler, gCurrentMove)
             && gMovesInfo[gCurrentMove].category == category
...
u32 GetMoveTarget(u16 move, u8 setTarget)
{
    ...
    case MOVE_TARGET_SELECTED:
        side = BATTLE_OPPOSITE(GetBattlerSide(gBattlerAttacker));
        if (IsAffectedByFollowMe(side, move))
        {
...
bool32 IsBattlerProtected(u32 battler, u32 move)
{
    if (gMovesInfo[move].ignoresProtect)
        ...
        return TRUE;
             ...
             && gMovesInfo[move].target & (MOVE_TARGET_BOTH | MOVE_TARGET_FOES_AND_ALLY))
        return TRUE;
...
static bool32 IsBattlerGroundedRingTarget(u32 battler, bool32 considerInverse)
{
    u32 holdEffect = GetBattlerHoldEffect(battler, TRUE);
...
u32 GetBattlerWeight(u32 battler)
{
    ...
    for (i = 0; i < gDisableStructs[battler].autotomiseCount; i++)
    {
        if (weight > 1000)
        {
            weight -= 1000;
...
static inline u32 CalcDefenseStat(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 isCrit, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectDef, u32 weather)
{
    ...
    if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_ROCK) && IsBattlerWeatherAffected(battlerDef, B_WEATHER_SANDSTORM) && !usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
...
static inline uq4_12_t GetMinimiseModifier(u32 move, u32 battlerDef)
{
    if (gMovesInfo[move].minimiseDoubleDamage && gStatuses3[battlerDef] & STATUS3_MINIMISED)
        return UQ_4_12(2.0);
    return UQ_4_12(1.0);
}
                                         ...
                                         u32 abilityAtk, u32 abilityDef, u32 holdEffectAtk, u32 holdEffectDef)
{
    ...
    DAMAGE_MULTIPLY_MODIFIER(GetMinimiseModifier(move, battlerDef));
...
{
    uq4_12_t mod = sTypeEffectivenessTable[moveType][defType];
...
s32 GetStealthHazardDamageByTypesAndHP(u8 hazardType, u8 type1, u8 type2, u32 maxHp)
{
    ...
    modifier = uq4_12_multiply(modifier, sTypeEffectivenessTable[hazardType][type1]);
    if (type2 != type1)
        modifier = uq4_12_multiply(modifier, sTypeEffectivenessTable[hazardType][type2]);
