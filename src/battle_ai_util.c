...
bool32 MovesWithCategoryUnusable(u32 attacker, u32 target, u32 category)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (moves[i] != MOVE_NONE
            ...
            && gMovesInfo[moves[i]].category == category
    ...
    default:
    {
        for (i = 0; i < gMovesInfo[move].numAdditionalEffects; i++)
        {
            switch (gMovesInfo[move].additionalEffects[i].moveEffect)
            {
                ...
                case MOVE_EFFECT_ACC_PLUS_2:
                case MOVE_EFFECT_ALL_STATS_UP:
                    if (noOfHitsToKo != 1 && DoesBattlerIgnoreAbilityChecks(abilityAtk, move))
                        return TRUE;
                    break;
            }
        }
...
{
    if (!gMovesInfo[move].damagesAirborne && gStatuses3[battlerDef] & STATUS3_ON_AIR)
        return TRUE;
...
bool32 HasOnlyMovesWithCategory(u32 battlerId, u32 category, bool32 onlyOffensive)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (onlyOffensive && IS_MOVE_STATUS(moves[i]))
            continue;
        if (moves[i] != MOVE_NONE && moves[i] != MOVE_UNAVAILABLE && gMovesInfo[moves[i]].category != category)
            return FALSE;
    }
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if (!(gBitTable[i] & moveLimitations))
        {
            if (ignoreStatus && IS_MOVE_STATUS(moves[i]))
              ...
              || gMovesInfo[moves[i]].target & (MOVE_TARGET_USER | MOVE_TARGET_OPPONENTS_FIELD))
                continue;
...
static bool32 PartyBattlerShouldAvoidHazards(u32 currBattler, u32 switchBattler)
{
    ...
    u32 flags = gSideStatuses[GetBattlerSide(currBattler)] & (SIDE_STATUS_SPIKES | SIDE_STATUS_STEALTH_ROCK | SIDE_STATUS_TOXIC_SPIKES);
    s32 hazardDamage = 0;
...
void IncreaseBurnScore(u32 battlerAtk, u32 battlerDef, u32 move, s32 *score)
{
    ...
    {
        (*score) += WEAK_EFFECT; // burning is good
        if (HasMoveWithCategory(battlerDef, DAMAGE_CATEGORY_PHYSICAL))
        {
            if (CanTargetFaintAi(battlerDef, battlerAtk))
                (*score) += DECENT_EFFECT; // burning the target to stay alive is cool
        }
