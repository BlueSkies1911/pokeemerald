...
#define OaksSpeechPrintMessage(str, speed) ({ \
    ...
    { \
        AddTextPrinterParameterized2(0, FONT_NORMAL, str, speed, NULL, 2, 1, 3); \
    } \
...
static void Task_OakSpeech12(u8 taskId)
{
    ...
    if (!IsTextPrinterActive(0))
    {
        ...
        CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 0x64, 0x42, 0, 0, 32, 0xFFFF1FFF, SPECIES_NIDORAN_F);
...
static void Task_OakSpeech19(u8 taskId)
{
    if (!IsTextPrinterActive(0))
    {
        ...
        DrawStdFrameWithCustomTileAndPalette(gTasks[taskId].data[13], 1, GetStandardFrameBaseTileNum(), 14);
        ...
        InitMenuNormal(gTasks[taskId].data[13], 2, 0, 1, GetFontAttribute(2, 1) + 2, 2, 0);
...
static void Task_OakSpeech26(u8 taskId)
{
    ...
    if (!gPaletteFade.active)
    {
        ...
        else if (!IsTextPrinterActive(0))
        {
            ...
            {
                CreateYesNoMenu(&sNewGameAdventureIntroWindowTemplates[2], GetStandardFrameBaseTileNum(), 14, 0);
...
static void Task_OakSpeech39(u8 taskId)
{
    ...
    if (sOakSpeechResources->unk_0012 % 20 == 0)
    {
        ...
        y = MathUtil_Inv16(tTrainerPicFadeState - 16);
...
static void CreateNidoranFSprite(u8 taskId)
{
    ...
    spriteId = NewGameOakSpeech_CreateNidoranFSprite(0x60, 0x60);
...
static void PrintNameChoiceOptions(u8 taskId, u8 hasPlayerBeenNamed)
{
    ...
    DrawStdFrameWithCustomTileAndPalette(data[13], 1, GetStandardFrameBaseTileNum(), 14);
    FillWindowPixelBuffer(gTasks[taskId].data[13], 0x11);
    ...
    InitMenuNormal(data[13], 2, 0, 1, 16, 5, 0);
