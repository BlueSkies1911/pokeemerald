    ...
    while (bits != 0x3F) // All mons were checked.
    {
        ...
        for (i = firstId; i < lastId; i++)
        {
            if (!(gBitTable[i] & invalidMons) && !(gBitTable[i] & bits))
            {
                ...
                typeEffectiveness *= sTypeEffectivenessTable[atkType1][defType1];
                if (atkType2 != atkType1)
                    typeEffectiveness *= sTypeEffectivenessTable[atkType2][defType1];
                if (defType2 != defType1)
                {
                    typeEffectiveness *= sTypeEffectivenessTable[atkType1][defType2];
                    if (atkType2 != atkType1)
                        typeEffectiveness *= sTypeEffectivenessTable[atkType2][defType2];
                }
