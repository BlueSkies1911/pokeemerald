...
static bool32 HasBadOdds(u32 battler, bool32 emitResult)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        if (aiMove != MOVE_NONE)
        {
            ...
            || aiMoveEffect == EFFECT_SPIKES || aiMoveEffect == EFFECT_TOXIC_SPIKES || aiMoveEffect == EFFECT_STEALTH_ROCK || aiMoveEffect == EFFECT_LEECH_SEED
    ...
    typeEffectiveness = uq4_12_multiply(typeEffectiveness, sTypeEffectivenessTable[atkType1][defType1]);
    if (atkType2 != atkType1)
        typeEffectiveness = uq4_12_multiply(typeEffectiveness, sTypeEffectivenessTable[atkType2][defType1]);
    if (defType2 != defType1)
    {
        typeEffectiveness = uq4_12_multiply(typeEffectiveness, sTypeEffectivenessTable[atkType1][defType2]);
        if (atkType2 != atkType1)
            typeEffectiveness = uq4_12_multiply(typeEffectiveness, sTypeEffectivenessTable[atkType2][defType2]);
    }
    ...
    if (hazardDamage > battlerHp)
    {
        ...
        for (i = firstId; i < lastId; i++)
        {
            ...
            for (j = 0; j < MAX_MON_MOVES; j++)
            {
                ...
                if (aiMove == MOVE_RAPID_SPIN || aiMove == MOVE_DEFOG)
                {
...
static u32 GetSwitchinHazardsDamage(u32 battler, struct BattlePokemon *battleMon)
{
    ...
    u32 hazardFlags = gSideStatuses[GetBattlerSide(battler)] & (SIDE_STATUS_SPIKES | SIDE_STATUS_STEALTH_ROCK | SIDE_STATUS_TOXIC_SPIKES | SIDE_STATUS_SAFEGUARD);

    // Check ways mon might avoid all hazards
    if (ability != ABILITY_MAGIC_GUARD)
    {
        // Stealth Rock
        if (hazardFlags & SIDE_STATUS_STEALTH_ROCK)
...
static u32 GetBestMonIntegrated(struct Pokemon *party, int firstId, int lastId, u32 battler, u32 opposingBattler, u8 battlerIn1, u8 battlerIn2, bool32 isSwitchAfterKO)
{
    ...
    else
    {
        ...
        // If ace mon is the last available Pokemon and U-Turn/Volt Switch was used - switch to the mon.
            ...
            && gMovesInfo[gLastUsedMove].effect == EFFECT_HIT_ESCAPE)
            return aceMonId;
    }
    return PARTY_SIZE;
}
