...
static void DoTVShowKantoTreasureInvestigators(void);
...
static void DoTVShowWhatsNo1InKantoToday(void);
...
static const u8 *const sTVKantoTreasureInvestisatorsTextGroup[] = {
    gTVKantoTreasureInvestigatorsText00,
    gTVKantoTreasureInvestigatorsText01,
    gTVKantoTreasureInvestigatorsText02
};
...
static const u8 *const sTVWhatsNo1InKantoTodayTextGroup[] = {
    gTVWhatsNo1InKantoTodayText00,
    gTVWhatsNo1InKantoTodayText01,
    gTVWhatsNo1InKantoTodayText02,
    gTVWhatsNo1InKantoTodayText03,
    gTVWhatsNo1InKantoTodayText04,
    gTVWhatsNo1InKantoTodayText05,
    gTVWhatsNo1InKantoTodayText06,
    gTVWhatsNo1InKantoTodayText07,
    gTVWhatsNo1InKantoTodayText08
};
...
//  case PLAYERS_HOUSE_TV_NONE:
    default:
        if ((gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(CERULEAN_CITY_HOUSE4)
         && gSaveBlock1Ptr->location.mapNum == MAP_NUM(CERULEAN_CITY_HOUSE4)))
        {
            // NPC in Cerulean City House4 is always watching TV
            SetTVMetatilesOnMap(width, height, METATILE_Building_TV_On);
        }
        else if ((VarGet(VAR_TEMP_1) == 1) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(SAFFRON_CITY_POKEMON_TRAINER_FAN_CLUB))
        {
            // NPC in Saffron Trainer Fan Club is watching TV when not a fan
            SetTVMetatilesOnMap(width, height, METATILE_Building_TV_On);
        }
        else if ((gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(PLAYERS_HOUSE_2F)
         && gSaveBlock1Ptr->location.mapNum == MAP_NUM(PLAYERS_HOUSE_2F)))
        {
            // TV in Players Room is never on unless playing NES
            SetTVMetatilesOnMap(width, height, METATILE_Building_TV_Off);
        }
...
void TryPutTodaysRivalTrainerOnAir(void)
{
    ...
    if (sCurTVShowSlot != -1)
    {
        ...
        if (IsNationalPokedexEnabled())
            ...
            show->rivalTrainer.dexCount = GetKantoPokedexCount(FLAG_GET_CAUGHT);
...
u8 CheckForPlayersHouseNews(void)
{
    ...
    if (gSaveBlock1Ptr->location.mapNum != MAP_NUM(PLAYERS_HOUSE_1F))
        return PLAYERS_HOUSE_TV_NONE;

    if (FlagGet(FLAG_SYS_TV_HOME) == FALSE)
...
void GetMomOrDadStringForTVMessage(void)
{
    ...
    if (gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(PLAYERS_HOUSE_1F)
         && gSaveBlock1Ptr->location.mapNum == MAP_NUM(PLAYERS_HOUSE_1F))
    {
...
void SanitizeTVShowLocationsForRuby(TVShow *shows)
{
    ...
    for (i = 0; i < LAST_TVSHOW_IDX; i++)
    {
        ...
        {
        case TVSHOW_WORLD_OF_MASTERS:
            if (shows[i].worldOfMasters.location > HOENN_MAPSEC_START)
                memset(&shows[i], 0, sizeof(TVShow));
            break;
        case TVSHOW_POKEMON_TODAY_FAILED:
            if (shows[i].pokemonTodayFailed.location > HOENN_MAPSEC_START)
                memset(&shows[i], 0, sizeof(TVShow));
            break;
        }
    }
...
void DoTVShow(void)
{
    if (gSaveBlock1Ptr->tvShows[gSpecialVar_0x8004].common.active)
    {
        ...
        case TVSHOW_TREND_WATCHER:
            ...
            break;
        case TVSHOW_TREASURE_INVESTIGATORS:
            DoTVShowKantoTreasureInvestigators();
            break;
        case TVSHOW_FIND_THAT_GAMER:
...
static void DoTVShowKantoTreasureInvestigators(void)
{
    ...
    ShowFieldMessage(sTVKantoTreasureInvestisatorsTextGroup[state]);
}
...
static void DoTVShowWhatsNo1InKantoToday(void)
{
    ...
    ShowFieldMessage(sTVWhatsNo1InKantoTodayTextGroup[state]);
}
