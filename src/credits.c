...
static const struct CreditsOverworldCmd sOverworldCmd_Route21[] = {
    CREDITSOVWLDLOADMAP(ROUTE21, 8, 20, 1),
    CREDITSOVWLDSCROLL(0, -1, 0x0500), // Scroll up
    CREDITSOVWLDEND,
};
...
static s32 RollCredits(void)
{
    ...
    case CREDITSSCENE_PRINT_TITLE_STAFF:
        ...
        AddTextPrinterParameterized4(sCreditsMgr->windowId, FONT_SHORT, 0x08, 0x29, 1, 2, sTextColor_Header, 0, TITLE_TEXT);
    ...
    case CREDITSSCENE_PRINT_ADDPRINTER1:
        ...
        AddTextPrinterParameterized4(sCreditsMgr->windowId, FONT_SHORT, 2, 6, 0, 0, sTextColor_Header, -1, sCreditsTexts[sCreditsScript[sCreditsMgr->scrcmdidx].param].title);
        sCreditsMgr->mainseqno = CREDITSSCENE_PRINT_ADDPRINTER2;
        return sCreditsMgr->canSpeedThrough;
    case CREDITSSCENE_PRINT_ADDPRINTER2:
        win0v[0] = sCreditsTexts[sCreditsScript[sCreditsMgr->scrcmdidx].param].unused;
        AddTextPrinterParameterized4(sCreditsMgr->windowId, FONT_SHORT, 8, 6, 0, 0, sTextColor_Regular, -1, sCreditsTexts[sCreditsScript[sCreditsMgr->scrcmdidx].param].names);
