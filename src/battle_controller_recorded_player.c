...
static void ChooseActionInBattlePalace(u32 battler)
{
    ...
    {
        BtlController_EmitTwoReturnValues(battler, BUFFER_B, RecordedBattle_GetBattlerAction(battler), 0);
        RecordedPlayerBufferExecCompleted(battler);
    }
}
...
static void RecordedPlayerHandleChooseMove(u32 battler)
{
    ...
    {
        u8 moveId = RecordedBattle_GetBattlerAction(battler);
        u8 target = RecordedBattle_GetBattlerAction(battler);
...
static void RecordedPlayerHandleChooseItem(u32 battler)
{
    u8 byte1 = RecordedBattle_GetBattlerAction(battler);
    u8 byte2 = RecordedBattle_GetBattlerAction(battler);
    gBattleStruct->chosenItem[battler] = (byte1 << 8) | byte2;
    gBattleStruct->itemPartyIndex[battler] = RecordedBattle_GetBattlerAction(battler);
    gBattleStruct->itemMoveIndex[battler] = RecordedBattle_GetBattlerAction(battler);
