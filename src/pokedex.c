...
#define temp_dexNum     vars[2]
    ...
    case ORDER_NUMERICAL:
        if (temp_isKantoDex)
        {
            ...
            {
                temp_dexNum = KantoToNationalOrder(i + 1);
    ...
    case ORDER_ALPHABETICAL:
        for (i = 0; i < ARRAY_COUNT(gPokedexOrder_Alphabetical); i++)
        {
            ...
            if (temp_dexNum <= NATIONAL_DEX_COUNT && (!temp_isKantoDex || NationalToKantoOrder(temp_dexNum) != 0) && GetSetPokedexFlag(temp_dexNum, FLAG_GET_SEEN))
            {
    ...
    case ORDER_HEAVIEST:
        for (i = ARRAY_COUNT(gPokedexOrder_Weight) - 1; i >= 0; i--)
        {
            ...
            if (temp_dexNum <= NATIONAL_DEX_COUNT && (!temp_isKantoDex || NationalToKantoOrder(temp_dexNum) != 0) && GetSetPokedexFlag(temp_dexNum, FLAG_GET_CAUGHT))
            {
                sPokedexView->pokedexList[sPokedexView->pokemonListCount].dexNum = temp_dexNum;
                sPokedexView->pokedexList[sPokedexView->pokemonListCount].seen = TRUE;
...
static void CreateMonDexNum(u16 entryNum, u8 left, u8 top, u16 unused)
{
    ...
    if (sPokedexView->dexMode == DEX_MODE_KANTO)
        dexNum = NationalToKantoOrder(dexNum);
...
static void PrintMonInfo(u32 num, u32 value, u32 owned, u32 newEntry)
{
    ...
    if (value == 0)
        value = NationalToKantoOrder(num);
    else
        value = num;
    ...
    PrintInfoScreenText(name, 132, 0x19);
