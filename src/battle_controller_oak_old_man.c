...
static void OakOldManBufferRunCommand(void)
{
    ...
    {
        if (gBattleResources->bufferA[gActiveBattler][0] < NELEMS(sOakOldManBufferCommands))
            sOakOldManBufferCommands[gBattleResources->bufferA[gActiveBattler][0]]();
        else
...
static void OakOldManHandleDrawTrainerPic(void)
{
    if (gBattleTypeFlags & BATTLE_TYPE_FIRST_BATTLE)
    {
        DecompressTrainerBackPic(TRAINER_BACK_PIC_RED +  gSaveBlock2Ptr->playerGender, gActiveBattler);
        SetMultiuseSpriteTemplateToTrainerBack(TRAINER_BACK_PIC_RED +  gSaveBlock2Ptr->playerGender, GetBattlerPosition(gActiveBattler));
        gBattlerSpriteIds[gActiveBattler] = CreateSprite(&gMultiuseSpriteTemplate,
                                                         80,
                                                         (8 - gTrainerBackPicCoords[TRAINER_BACK_PIC_RED + gSaveBlock2Ptr->playerGender].size) * 4 + 80,
                                                         30);
    }
    else
    {
        DecompressTrainerBackPic(TRAINER_BACK_PIC_OLDMAN, gActiveBattler);
        SetMultiuseSpriteTemplateToTrainerBack(TRAINER_BACK_PIC_OLDMAN, GetBattlerPosition(gActiveBattler));
        gBattlerSpriteIds[gActiveBattler] = CreateSprite(&gMultiuseSpriteTemplate,
                                                         80,
                                                         (8 - gTrainerBackPicCoords[TRAINER_BACK_PIC_OLDMAN].size) * 4 + 80,
                                                         30);
    }
...
static void OakOldManHandleChooseItem(void)
{
    ...
    for (i = 0; i < 3; ++i)
        gBattlePartyCurrentOrder[i] = gBattleResources->bufferA[gActiveBattler][i + 1];
}
...
static void OakOldManHandleIntroTrainerBallThrow(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_FIRST_BATTLE)
    {
        ...
        LoadCompressedPalette(gTrainerBackPicPaletteTable[TRAINER_BACK_PIC_RED + gSaveBlock2Ptr->playerGender].data, 0x100 + paletteNum * 16, 32);
