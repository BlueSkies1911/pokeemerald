...
static void SetBattlerAiData(u8 battlerId)
{
    ...
    AI_DATA->holdEffectParams[battlerId] = ItemId_GetHoldEffectParam(gBattleMons[battlerId].item);
    ...
    // other
        ...
        case EFFECT_MIRROR_COAT:
            if (IsBattlerIncapacitated(battlerDef, AI_DATA->abilities[battlerDef]) || gBattleMons[battlerDef].status2 & (STATUS2_INFATUATION | STATUS2_CONFUSION))
                score--;
            if (predictedMove == MOVE_NONE || gBattleMoves[predictedMove].split == SPLIT_STATUS
        ...
        case EFFECT_FAKE_OUT:
            ...
            {
                if ((AI_DATA->holdEffects[battlerAtk] == HOLD_EFFECT_CHOICE_BAND)
        ...
        case EFFECT_SOLAR_BEAM:
            if (AI_DATA->holdEffects[battlerAtk] == HOLD_EFFECT_POWER_HERB
              || (AI_WeatherHasEffect() && gBattleWeather & B_WEATHER_SUN))
                break;
        ...
        case EFFECT_WORRY_SEED:
            if (AI_DATA->abilities[battlerDef] == ABILITY_INSOMNIA
              || AI_DATA->abilities[battlerDef] == ABILITY_TRUANT)
                score -= 10;
            break;
        ...
        case EFFECT_MAGNET_RISE:
            if (gFieldStatuses & STATUS_FIELD_GRAVITY
              ...
              || gStatuses3[battlerAtk] & (STATUS3_ROOTED | STATUS3_MAGNET_RISE)
...
// stat lowering effects
    ...
    case EFFECT_REST:
        ...
        else if (ShouldRecover(battlerAtk, battlerDef, move, 100))
        {
            if (AI_DATA->holdEffects[battlerAtk] == HOLD_EFFECT_CURE_SLP
              ...
              || (gBattleWeather & B_WEATHER_RAIN && gWishFutureKnock.weatherDuration != 1 && AI_DATA->abilities[battlerAtk] == ABILITY_HYDRATION))
            {
                score += 2;
            }
            else
    ...
    case EFFECT_FOLLOW_ME:
        if (isDoubleBattle
          ...
          && (IsAffectedByPowder(battlerDef, AI_DATA->abilities[battlerDef], AI_DATA->holdEffects[battlerDef]))
          && IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
        {
    ...
    case EFFECT_TRICK:
        ...
        case HOLD_EFFECT_EJECT_BUTTON:
            ...
            break;
        default:
            if (AI_DATA->items[battlerAtk] == ITEM_NONE)
            {
                switch (AI_DATA->holdEffects[battlerDef])
                {
                case HOLD_EFFECT_CHOICE_BAND:
        ...
        break;
    case EFFECT_ROLE_PLAY:
        if (!IsRolePlayBannedAbility(AI_DATA->abilities[battlerDef])
    ...
    case EFFECT_TICKLE:
        if (gBattleMons[battlerDef].statStages[STAT_DEF] > 4 && HasMoveWithSplit(battlerAtk, SPLIT_PHYSICAL)
          && ShouldLowerDefense(battlerAtk, battlerDef, AI_DATA->abilities[battlerDef]))
        {
            score += 2;
        }
    ...
    case EFFECT_COUNTER:
        if (!IsBattlerIncapacitated(battlerDef, AI_DATA->abilities[battlerDef]) && predictedMove != MOVE_NONE)
        {
            ...
            if (GetMoveDamageResult(predictedMove) >= MOVE_POWER_GOOD && gBattleMoves[predictedMove].split == SPLIT_PHYSICAL)
                score += 3;
        }
        break;
    case EFFECT_MIRROR_COAT:
        if (!IsBattlerIncapacitated(battlerDef, AI_DATA->abilities[battlerDef]) && predictedMove != MOVE_NONE)
        {
            if (gDisableStructs[battlerDef].tauntTimer != 0)
                score++;    // target must use damaging move
            if (GetMoveDamageResult(predictedMove) >= MOVE_POWER_GOOD && gBattleMoves[predictedMove].split == SPLIT_SPECIAL)
                score += 3;
        }
        break;
    case EFFECT_FLAIL:
...
static s16 AI_HPAware(u8 battlerAtk, u8 battlerDef, u16 move, s16 score)
{
    ...
    if (IsTargetingPartner(battlerAtk, battlerDef))
    {
        if ((moveType == TYPE_ELECTRIC && AI_DATA->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_VOLT_ABSORB)
